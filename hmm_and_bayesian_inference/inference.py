...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        --Jail Edge Case--
        Check if the noisy distance is None, which means the ghost has been captured by Pacman. 
        If so, update all beliefs so that the ghost appears in its prison cell
        """
        if noisyDistance is None:
            ...
            return
        """
        Iterate over all possible positions and update the beliefs using the observation likelihood
        """
        allPossible = util.Counter()
        pacmanPosition = gameState.getPacmanPosition()
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            likelihood = busters.getObservationDistribution(noisyDistance)[trueDistance]
            allPossible[p] = self.beliefs[p] * likelihood
        # Normalizing the belief states
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Updating the new beliefs based on the transition model:
